var bingo = {
	...
	init: function(socket){
		...
		socket.on("check_number", function (data) {
			_this.where_is_it(data.num);
			_this.print_msg(data.username + "님이 '" + data.num + "'을 선택했습니다.");
		});
		
		socket.on("game_started", function(data){
			console.log("enter the game_started");
			_this.print_msg(data.username + " 님이 게임을 시작했습니다.");
			$("#start_button").hide();
		});
		...
		socket.on("update_users", function (data, user_count) {
			console.log(data);
			user_cnt = user_count;
			_this.update_userlist(data, socket);
		});
		...
		$("table.bingo-board td").each(function (i) {
			...
			$(this).click(function (){
				if(user_cnt == 1){
					_this.print_msg("<알림> 최소 2명부터 게임이 가능합니다.");
				}
				else{
					_this.select_num(this, socket);
				}
			});
		});
